#ifndef CADMIUM_EXAMPLE_DEVSTONE_COUPLED_HO_HPP_
#define CADMIUM_EXAMPLE_DEVSTONE_COUPLED_HO_HPP_

#include "devstone_coupled.hpp"

namespace cadmium::example::devstone {
	//! HO DEVStone model class.
	struct DEVStoneCoupledHO : public DEVStoneCoupled {
		/**
		 * HO model constructor function.
		 * @param width width of the HO DEVStone model.
		 * @param depth depth of the HO DEVStone model.
		 * @param intDelay internal delay for the DEVStone atomic models.
		 * @param extDelay external delay for the DEVStone atomic models.
		 */
		DEVStoneCoupledHO(int width, int depth, int intDelay, int extDelay);
	};
}  //namespace cadmium::example::devstone

#endif //CADMIUM_EXAMPLE_DEVSTONE_COUPLED_HO_HPP_
