...
#ifndef _CADMIUM_CORE_SIMULATION_FLAT_COORDINATOR_HPP_
#define _CADMIUM_CORE_SIMULATION_FLAT_COORDINATOR_HPP_
...
namespace cadmium {
    ...
    class FlatCoordinator : public FlatAbstractSimulator {
     private:
        std::shared_ptr<Coupled> model;
        //std::vector<std::shared_ptr<FlatAbstractSimulator>> simulators;
        std::vector<std::shared_ptr<FlatSimulator>> simulators;

     public:
        FlatCoordinator(std::shared_ptr<Coupled> model, double time): FlatAbstractSimulator(time), model(std::move(model)) {

        	if (this->model == nullptr) {
				throw std::bad_exception();  // TODO custom exceptions
			}
			for (auto& component: this->model->getComponents()) {
			    std::shared_ptr<FlatSimulator> simulator;
//			    auto coupled = std::dynamic_pointer_cast<Coupled>(component);
//			    if (coupled != nullptr) {
//				    simulator = std::make_shared<FlatCoordinator>(coupled, time);
//			    } else {
				    auto atomic = std::dynamic_pointer_cast<AbstractAtomic>(component);
				    if (atomic == nullptr) {
					    throw std::bad_exception();  // TODO custom exceptions
				    }
				    simulator = std::make_shared<FlatSimulator>(atomic, time);
//				}
			    simulators.push_back(simulator);
			    timeNext = std::min(timeNext, simulator->timeNext);
			}

		//simulators = subcoordinators;
/*
...
*/
		...
		double execute_state_transitions(double time) {
			...
			for (auto& simulator: simulators) {
                simulator->execute_state_transition(time);
                timeNext = std::min(timeNext, simulator->timeNext);
			}
			return timeNext;
		}
