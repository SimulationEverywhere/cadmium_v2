...
#ifndef _CADMIUM_CORE_SIMULATION_FLAT_SIMULATOR_HPP_
#define _CADMIUM_CORE_SIMULATION_FLAT_SIMULATOR_HPP_
...
namespace cadmium {
    class FlatSimulator: public FlatAbstractSimulator {
     private:
        std::shared_ptr<AbstractAtomic> model;

     public:
        FlatSimulator(std::shared_ptr<AbstractAtomic> model, double time): FlatAbstractSimulator(time), model(std::move(model)) {
			if (this->model == nullptr) {
				throw std::bad_exception(); // TODO custom exceptions
			}
			timeNext = timeLast + this->model->timeAdvance();
        }
        ~FlatSimulator() override = default;
        		...
        		void execute_state_transition(double time) {
        			...
        			if (logger != nullptr) {
        				...
        				if (time >= timeNext) {
        					...
        					model->getOutPorts().logMessages(logger, time, modelId, model->getId());
        				}
