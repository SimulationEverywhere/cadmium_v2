...
public:
    emergency_area(std::string id, nlohmann::json j_params): cadmium::Atomic<emergency_area_state>(std::move(id), "emergency_area", emergency_area_state()) {
		...
		population = j_params.at("population").get<int>();
		n_ports = 3;
        ...
        double count = 1.9 / 1000 * population;

        emergency_max = int(count + 0.5 - (count < 0));

		// generate an initial count for the first emergency to output.
    	// It will be output before the first internal transition
        state.quantity = (emergency_max == 1) ? 1 : std::rand() % (emergency_max - 1) + 1;
    }

    void internalTransition(emergency_area_state& s) const override {
    	if (s.quantity == 0 && emergency_max > 0) {
    		s.port_i = 1;
    		s.quantity = std::rand() % (emergency_max) + 1;
    	}

    	else if (s.port_i > n_ports) {
			s.dead += s.quantity;
    		s.port_i = 1;
    		s.quantity = 0;
    	}
    ...
    void output(const emergency_area_state& s, const cadmium::PortSet& y) const override {
    	...
    	if (s.quantity == 0 || s.port_i > n_ports) return;
    	...
    	emergency_t em = emergency_t(id, s.port_i, s.quantity);
