#ifndef CADMIUM_TEMPLATE_CELLDEVS_ASYMM_CELL_HPP_
#define CADMIUM_TEMPLATE_CELLDEVS_ASYMM_CELL_HPP_
...
void from_json(const nlohmann::json& j, CellConfig& c) {}
 ...
 class CellTemplate : public cadmium::celldevs::AsymmCell<State, Vicinity> {
	 CellConfig cellConfig;   /// additional cell configuration (remove it if you don't need it)
 public:
	 CellTemplate(const std::string& id, const std::shared_ptr<const cadmium::celldevs::AsymmCellConfig<State, Vicinity>>& config):
	   cadmium::celldevs::AsymmCell<State, Vicinity>(id, config), cellConfig() {
	  ...
	  */
	[[nodiscard]] State localComputation(State state, const std::unordered_map<std::string, cadmium::celldevs::NeighborData<State, Vicinity>>& neighborhood, const cadmium::PortSet& x) const override {
...
#endif //CADMIUM_TEMPLATE_CELLDEVS_ASYMM_CELL_HPP_
